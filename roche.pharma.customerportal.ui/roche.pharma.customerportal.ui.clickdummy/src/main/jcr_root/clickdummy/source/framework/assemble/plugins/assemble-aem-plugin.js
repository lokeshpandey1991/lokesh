...
module.exports = function(params, callback) {
  ...
  // uxlibContentRoot: '<%= config.app %>/jcr_root/content/gbp-uxlib-dev',
  // uxlibContentSection: 'pages',
  // designJcrPath: '/etc/designs/gbp'
  ...
  grunt.file.write(path.join(aemuxlib.contentRoot,".content.xml"), buildDemoRootPageContentDescriptor('roche-ux'));

  async.forEachSeries(pages, function(file, next) {

    if (page.src !== file.src) {next(); return;}
    //var outputDir = path.join(aem.dest, path.dirname(file.src), file.basename);
    ...
    //console.log('in ' + path.dirname(file.src) + ' data '+ JSON.stringify(dataJson));
    ...
    //var demoPageFolder = path.join(aemuxlib.contentRoot,componentResourcePath.replace(aemuxlib.componentsJcrRoot, '/'), aemuxlib.contentSection,file.basename);
