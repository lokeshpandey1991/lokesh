...
(function (window, $, snro) {
  ...
  function _defineCustomEvents() {
    ...
    $(document).on('mouseenter focusin', '.x-overlay-link', function () {
      let parentList = $(this).parents('.x-overlay-link-container');
  ...
  let _initializeRedirectPopup = () => {
      ...
      $('body').on('click','.js-redirect-ok', function(){
        let targetLink = $('.js-redirect-popup').data('target');  
    ...
    _handleExternalLinks = () => {
      $('body').on('click','a', function(e) {
        e.preventDefault();
        let givenTarget = $(this).attr('href').trim();
        if(givenTarget){
  ...
  snro.commonUtils = {
    ...
    loader: function (target, appendMode, overlay=false) {
      target = target||'body';
      let loader, loadercol = $('<div>').addClass('loading-icon js-loading-icon').prepend('<span></span><span></span>');
      if(!overlay){
        loader = loadercol;
      }else{
