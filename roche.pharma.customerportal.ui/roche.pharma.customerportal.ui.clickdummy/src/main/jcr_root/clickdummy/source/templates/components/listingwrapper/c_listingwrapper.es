...
(function (window, $, snro) {
  ...
  let _cache = {'firstLoad': true},
    _populateAnalytics = function (data) {
      if (_cache.firstLoad) {
  ...
  snro.listingwrapperCmp = {
    ...
    updateQueryObject: function (queryObject) {
      let updatedString = _createQueryString(queryObject);
      window.history.replaceState('', document.title, window.location.pathname + updatedString );
    },

    updateCount: function (upperResult, total) {
      let currentCountString = _cache.currentCountText &&
        _cache.currentCountText
          .replace('{0}', upperResult || 0)
          .replace('{1}', total || 0);
    ...
    selectTemplate() {
      ...
      let selectedTabId = $('.js-tab-item.selected').data('tab-name'),
    ...
    renderTemplate(templateName, data) {
      if (templateName) {
        const resultTemplate = window.roche.templates[templateName];
     ...
     */
    renderResults(data) {
      let listingWrapper = this.getListingWrapper();
      ...
      if (data.results && data.results.data && data.results.data.length > 0) {
        ...
        let resultsHtml = this.renderTemplate(this.selectTemplate(), results);
    ...
    // bind dom events
    bindEvents() {
      let queryObject = this.getUrlParameter(),
        that = this, timeout,
    ...
    enablePLPFilters: function() {
      ...
      try {
        ...
        if(_cache.plpSSIProperties) {
          let ssiProperties = _cache.plpSSIProperties.facets[0].values,
            filterMarkup = '';
          for(let i=0; i<ssiProperties.length; i++) {
            filterMarkup+= '<a href="javascript:void(0)" class="c-plp-filters js-plp-filters x-sub-hover" ' +
              'data-value="'+ssiProperties[i].value+'">' +
              ssiProperties[i].displayKey +' </a>';
          }
