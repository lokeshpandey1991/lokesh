...
(function (window, $, snro) {
  ...
  function syncCloneImages() {
    let $carouselItemArray = $('.js-event-item');
    $carouselItemArray.each(function (index, item) {
      let origImgSrc = $(item).find('.js-event-asset').attr('src');
  ...
  snro.carouselComp = {
    ...
    initializeCarousel() {
      ...
      _cache.carouselContainer.on('beforeChange', function (event, slick, currentSlide, nextSlide) {
        ...
        let targetSlide = $(slick.$slides.get(currentSlide))[0];

        //if current slide has video - stop video before change
        let $jpPlayer = $(targetSlide).find('.jp-jplayer');
        ...
        let $activeSlides = $('.slick-slide.slick-active'),
        ...
        for (let i = 0; i <= activeSlides; i++) {
          let newIndex = ((nextSlide + i) < slideCount) ? (nextSlide + i) : (nextSlide + i - slideCount),
            $slideFragment = $(slick.$slides.get(newIndex));

          if (i === 0) {
            //$slideFragment.find('.js-asset-wrapper').addClass('slideInRight');
            $slideFragment.find('.js-asset-wrapper-cloned').removeClass('hidden')
              .find('.js-event-asset-cloned').attr('src', activeSlidesImagesArray[i]);
          }
        }
      });
      ...
      _cache.carouselContainer.on('afterChange', function (event, slick) {
        ...
        $('.slick-slide.slick-current .js-asset-wrapper').addClass('slideInRight')
