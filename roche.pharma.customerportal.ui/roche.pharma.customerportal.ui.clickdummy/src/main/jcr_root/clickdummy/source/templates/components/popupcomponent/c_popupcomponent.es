...
(function (window, $, snro) {
  ...
  snro.modalPopComp = {
    ...
    initializeModal() {
      let metaTagPersonaLevel = this.personaMetaTag,
    ...
    updateTemplate(modalBodyText,bool) {
    ...
    bindEvents() {
      ...
      $('body').on('click','.js-btn-secondary',function(){
        ...
        if(context.dLinkWithNoMatchPersona) {
          snro.commonUtils.setCookie('persona-type',
            context.personaMetaTag,
            Infinity);
          snro.personaPickerComp._checkCookie();
          return;
        }
        if(!context.isPersonaModalPopUp) {
          ...
          languageLocationCookie.replace('/','|'),
          Infinity);
