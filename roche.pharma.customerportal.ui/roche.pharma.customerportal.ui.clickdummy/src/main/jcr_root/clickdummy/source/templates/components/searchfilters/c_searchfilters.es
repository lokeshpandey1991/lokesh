...
(function (window, $, snro) {
  ...
  snro.searchFiltersCmp = {
    ...
    populateFacets: (filters) => {

      let facetTemplate = '<div class="c-filter__facet js-filter-facet">\n' +
      ...
      $.each(filters, function (index, val) {
        let facetId = Object.keys(val)[0],          //x1,x2 etc
          filterId = Object.keys(val)[1],           //q1,q2 etc
          facetValue = val[Object.keys(val)[0]],    //value of x1
        ...
        facetMarkup += facetTemplate
        .replace('{0}', facetId)
        .replace('{1}', filterId)
        .replace('{2}', facetValue)
        .replace('{3}', filterText);
      });
    ...
    bindFilterFunctions: () => {
      let indexIncrement = $('.js-page-props').length ? 2 : 1;
      _cache.filterContainerMobile
        .unbind('click')
        .bind('click', function ( ) {
      ...
      .bind('click', function () {
        ...
        $(facetContainer).each(function (i, e) {
          let temp = {}, index = i + indexIncrement;
      ...
      _cache.filterContainerMobileClose
        .unbind('click')
    ...
    renderFilters: (facets) => {
      let mdFacets = _self.removeLastNode(facets);
      let resultsFilters = window.roche.templates['searchFilters'](mdFacets);
