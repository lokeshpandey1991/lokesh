...
(function (window, $, snro) {
  ...
  function liipboxAnimation(event) {
    ...
    let toggleLiipbox = (event === 'close') > 0 ? true : false,

      // jquery selectors
      linkSelector = toggleLiipbox ? $('.js-liipbox').find('.link-row').get().reverse() : $(
        '.js-liipbox').find('.link-row'),
    ...
    if (linkSelector.length > 0) {
      $(linkSelector)
        .each(function (index, element) {
          let transitionDelay = (index * 0.04) + 's';
  ...
  snro.liipboxComp = {
    ...
    _accessibleLiipbox() {
      ...
      let liipbox = document.querySelector('.js-liipbox'),
      ...
      if (liipboxOverlay && $(liipboxOverlay).length > 0) {
        ...
        let closeButtons = liipboxOverlay.querySelector('.js-cross-icon');
