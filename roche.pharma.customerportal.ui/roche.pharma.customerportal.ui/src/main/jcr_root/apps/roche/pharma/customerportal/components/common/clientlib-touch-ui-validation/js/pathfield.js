 ...
 */
(function (ns, window, document, $, Granite) {
	...
	$.validator.register({
		...
		validate: function (el) {
			...
			value = el.val(),
			allowNonRootPath = ns.utils.getDataAttribute(container, 'allownonrootpath'),
			allowNonExistingPath = ns.utils.getDataAttribute(container, 'allownonexistingpath'),
			rootPath = container.data('root-path');

            if(allowNonRootPath === undefined && allowNonExistingPath === undefined){
				return null;
            }
			 ...
			 */
			if (!el.is(':focus') && !allowNonExistingPath && ns.utils.isNotBlank(value)) {
					status = (function () {
						return $.ajax({
							type: 'GET',
							url: value + '.json',
	 ...
	 * stroke.
	 */
	$(document).on('blur', PATHBROWSER_SELECTOR, function () {
		var $pathbrowser,
		container,
		value,
		...
		if (allowNonExistingPath !== undefined && !allowNonExistingPath && ns.utils.isNotBlank(value)) {
			status = (function () {
				return $.ajax({
					type: 'GET',
					url: value + '.json',
